...
mp_pool_t * mp_create_pool(size_t size)
{
    ...
    p = memalign(MP_ALIGNMENT, size + sizeof(mp_pool_t) + sizeof(mp_node_t));
    if (p == NULL) {
        return NULL;
    }
    ...
    p->head->last = (u_char *) p + sizeof(mp_pool_t) + sizeof(mp_node_t);
...
void mp_reset_pool(mp_pool_t *pool)
{
    ...
    for (h = pool->head; h; h = h->next) {
        h->last = (u_char *) h + sizeof(mp_node_t);
    }
...
static void * mp_alloc_block(mp_pool_t *pool, size_t size)
{
    ...
    h = pool->head;
    psize = (size_t) (h->end - (u_char *) h);

    m = memalign(MP_ALIGNMENT, psize);
    if (m == NULL) {
        return NULL;
    }
